...
CREATE INDEX idx_videos_template_id ON videos(template_id);
CREATE INDEX idx_videos_status ON videos(status);
CREATE INDEX idx_ad_templates_video_type ON ad_templates(video_type);
CREATE INDEX idx_ad_templates_is_active ON ad_templates(is_active);
...
-- RLS Policies for ad_templates (public read, admin write)
CREATE POLICY "Anyone can view active ad templates"
    ON ad_templates FOR SELECT
    USING (is_active = TRUE);

-- Note: Admin policies for ad_templates will be handled separately
-- For now, you'll need to insert templates directly or create an admin role
